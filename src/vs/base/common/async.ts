...
export function raceTimeout<T>(promise: Promise<T>, timeout: number, onTimeout?: () => void): Promise<T> {
	...
	return Promise.race([
		...
		new Promise<T>(resolve => promiseResolve = () => resolve(undefined as unknown as T))
	]);
}
...
export class Limiter<T> {
	...
	queue(factory: ITask<Promise<T>>): Promise<T> {
		...
		return new Promise<T>((c, e) => {
			this.outstandingPromises.push({ factory, c: (value?: T | Promise<T> | undefined) => c(value as T), e });
			this.consume();
		});
	}
