...
import { IProcessEnvironment, isWindows, isMacintosh, isLinux } from './base/common/platform';
import { normalizeDriveLetter } from './base/common/labels';
import { localize } from './nls';
import { URI as uri } from './base/common/uri';
// import { IConfigurationResolverService } from './workbench/services/configurationResolver/common/configurationResolver';
...
export class AbstractVariableResolverService {
	...
	resolve(root: IWorkspaceFolder | undefined, value: string): string;
	resolve(root: IWorkspaceFolder | undefined, value: string[]): string[];
	resolve(root: IWorkspaceFolder | undefined, value: IStringDictionary<string>): IStringDictionary<string>;
	resolve(root: IWorkspaceFolder | undefined, value: any): any {
		return this.recursiveResolve(root ? root.uri : undefined, value);
	}

	resolveAnyBase(workspaceFolder: IWorkspaceFolder | undefined, config: any, commandValueMapping?: IStringDictionary<string>, resolvedVariables?: Map<string, string>): any {
	...
	resolveAny(workspaceFolder: IWorkspaceFolder | undefined, config: any, commandValueMapping?: IStringDictionary<string>): any {
		return this.resolveAnyBase(workspaceFolder, config, commandValueMapping);
	}

	resolveAnyMap(workspaceFolder: IWorkspaceFolder | undefined, config: any, commandValueMapping?: IStringDictionary<string>): { newConfig: any, resolvedVariables: Map<string, string> } {
	...
	private resolveString(folderUri: uri | undefined, value: string, commandValueMapping: IStringDictionary<string> | undefined, resolvedVariables?: Map<string, string>): string {
		...
		const replaced = value.replace(AbstractVariableResolverService.VARIABLE_REGEXP, (match: string, variable: string) => {
			...
			const resolvedValue = this.evaluateSingleVariable(match, variable, folderUri, commandValueMapping);
