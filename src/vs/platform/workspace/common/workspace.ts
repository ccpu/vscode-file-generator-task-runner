...
// @ts-ignore
import { IWorkspaceIdentifier, IStoredWorkspaceFolder, isRawFileWorkspaceFolder, isRawUriWorkspaceFolder, ISingleFolderWorkspaceIdentifier } from '../../workspaces/common/workspaces';
...
export function toWorkspaceFolders(configuredFolders: IStoredWorkspaceFolder[], workspaceConfigFile: URI): WorkspaceFolder[] {
	...
	for (const configuredFolder of configuredFolders) {
		...
		if (uri) {
			// remove duplicates
			const comparisonKey = resources.getComparisonKey(uri);
